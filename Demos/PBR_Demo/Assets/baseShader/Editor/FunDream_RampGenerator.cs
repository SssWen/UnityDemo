...
public class FunDream_RampGenerator : EditorWindow
{
	[MenuItem(FunDream_Menu.MENU_PATH + "Ramp Generator", false, 500)]
	static void OpenTool()
	{
		GetWindowFunDream_();
	}

	static private FunDream_RampGenerator GetWindowFunDream_()
	{
		FunDream_RampGenerator window = GetWindow<FunDream_RampGenerator>(true, "FunDream_ : Ramp Generator", true);
	...
	{
		string path = EditorUtility.OpenFilePanel("FunDream_ Gradient Texture", FunDream_GradientManager.LAST_SAVE_PATH, "png");
		if(!string.IsNullOrEmpty(path))
		{
			FunDream_GradientManager.LAST_SAVE_PATH = System.IO.Path.GetDirectoryName(path);
