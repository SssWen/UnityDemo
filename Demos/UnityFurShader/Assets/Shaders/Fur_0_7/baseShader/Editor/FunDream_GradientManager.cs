...
public class FunDream_GradientManager
{
	static public string LAST_SAVE_PATH
	{
		get { return EditorPrefs.GetString("FunDream__GradientSavePath", Application.dataPath); }
		set { EditorPrefs.SetString("FunDream__GradientSavePath", value); }
	}
	...
	static public void SetGradientFromUserData(string userData, Gradient gradient)
	{
		...
		if(keys == null || keys.Length < 3 || keys[0] != "GRADIENT")
		{
			...
			Debug.LogError("[FunDream_GradientManager] Invalid Gradient Texture\nMake sure the texture was created with the Ramp Generator.");
			return;
		}
