...
public static class FunDream_GUI
{
	...
	static public GUIStyle HelpIcon
	{
		get
		{
			if(_HelpIcon == null)
			{
				...
				Texture2D icon = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__HelpIcon.png", typeof(Texture2D)) as Texture2D;
				Texture2D icon_down = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__HelpIcon_Down.png", typeof(Texture2D)) as Texture2D;
	...
	static public Texture2D EnabledBg
	{
		get
		{
			...
			{
				string rootPath = FunDream_Utils.FindReadmePath(true);
				_EnabledBg = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__EnabledBg.png", typeof(Texture2D)) as Texture2D;
				_EnabledBgPro = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__EnabledBgPro.png", typeof(Texture2D)) as Texture2D;
			}
	...
	static public GUIStyle CogIcon
	{
		get
		{
			if(_CogIcon == null)
			{
				...
				string rootPath = FunDream_Utils.FindReadmePath(true);
				Texture2D icon = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__CogIcon.png", typeof(Texture2D)) as Texture2D;
				Texture2D icon_down = AssetDatabase.LoadAssetAtPath("Assets" + rootPath + "/Editor/Icons/FunDream__CogIcon_Down.png", typeof(Texture2D)) as Texture2D;
				if(icon == null || icon_down == null) return null;
	...
	{
		if(FunDream_GUI.Button(rect, HelpIcon, "?", "Help about:\n" + helpTopic))
		{
...
public class FunDream_HeaderHelpDecorator : MaterialPropertyDrawer
{
	protected readonly string header;
	protected readonly string help;

	public FunDream_HeaderHelpDecorator(string header)
	{
		this.header = header;
		this.help = null;
	}
	public FunDream_HeaderHelpDecorator(string header, string help)
	{
		this.header = header;
		this.help = help;
	}
...
public class FunDream_HelpBoxDecorator : MaterialPropertyDrawer
{
	...
	{
		position.height -= 4f;
		EditorGUI.LabelField(position, GUIContent.none, new GUIContent(message, icon), FunDream_GUI.HelpBoxRichTextStyle);
	...
	public override float GetPropertyHeight(MaterialProperty prop, string label, MaterialEditor editor)
	{
		...
		var height = FunDream_GUI.HelpBoxRichTextStyle.CalcHeight(new GUIContent(message, icon), Screen.width - (scrollBar ? 51 : 34));
		return height + 6f;
	}
}
...
public class FunDream_SeparatorDecorator : MaterialPropertyDrawer
{
	public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
	{
		position.y += 4;
...
public class FunDream_OutlineNormalsGUIDrawer : MaterialPropertyDrawer
{
	...
	readonly string[] keywords = new string[] { "FunDream__NONE", "FunDream__COLORS_AS_NORMALS", "FunDream__TANGENT_AS_NORMALS", "FunDream__UV2_AS_NORMALS" };
...
public class FunDream_Vector4FloatsDrawer : MaterialPropertyDrawer
{
	...
	public FunDream_Vector4FloatsDrawer(string groupLabel, string labelX, string labelY, string labelZ, string labelW)
	{
	...
	public FunDream_Vector4FloatsDrawer(string groupLabel, string labelX, string labelY, string labelZ, string labelW, float minX, float maxX, float minY, float maxY, float minZ, float maxZ, float minW, float maxW)
	{
		this.groupLabel = groupLabel;
		this.labels = new string[] { labelX, labelY, labelZ, labelW };
...
public class FunDream_GradientDrawer : MaterialPropertyDrawer
{
...
public class FunDream_HeaderDecorator : MaterialPropertyDrawer
{
	protected readonly string header;

	public FunDream_HeaderDecorator(string header)
	{
		this.header = header;
	}
...
public class FunDream_KeywordFilterDrawer : MaterialPropertyDrawer
{
	protected readonly string[] keywords;

	public FunDream_KeywordFilterDrawer(string keyword)
	{
...
internal class FunDream_ToggleNoKeywordDrawer : MaterialPropertyDrawer
{
	...
	{
		float result;
		if(!FunDream_ToggleNoKeywordDrawer.IsPropertyTypeSuitable(prop))
		{
			result = 40f;
		}
		else
