...
v2f vert_base(appdata_base v)
{
    ...
    //     half3 direction = lerp(v.normal, _ForceLocal * _GravityStrength + v.normal * (1 - _GravityStrength), FURSTEP);
...
fixed4 frag_base(v2f i): SV_Target
{
    ...
    albedo = lerp(albedo,albedo*layerColor*noise.r,noise.r);
    ...
    fixed3 color = ambient + diffuse*attenuation + specular;
...
float4 frag_add(v2f i) : SV_Target { 
    ...
    fixed3 worldNormal = normalize(i.worldNormal);
    // fixed3 worldLight = normalize(_WorldSpaceLightPos0.xyz);
    float3 worldLight = normalize(lerp(_WorldSpaceLightPos0.xyz, _WorldSpaceLightPos0.xyz - i.worldPos.xyz,_WorldSpaceLightPos0.w));
    fixed3 worldView = normalize(_WorldSpaceCameraPos.xyz - i.worldPos.xyz);
    fixed3 worldHalf = normalize(worldView + worldLight);
    ...
    fixed3 diffuse = _LightColor0.rgb * albedo * saturate(dot(worldNormal, worldLight))*attenuation;
    fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(saturate(dot(worldNormal, worldHalf)), _Shininess);
    specular = 0;
