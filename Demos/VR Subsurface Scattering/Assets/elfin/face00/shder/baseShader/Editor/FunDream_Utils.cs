...
// General helper functions for FunDream_

public static class FunDream_Utils
{
	//--------------------------------------------------------------------------------------------------------------------------------
	...
	static public string ToShader (this TextureChannel channel)
	{
		switch(channel)
		{
			...
			default: Debug.LogError("[FunDream_Utils] Unrecognized texture channel: " + channel.ToShader()); return null;
		}
	}
	...
	static public TextureChannel FromShader(string str)
	{
		...
		switch(str)
		{
			...
			default: Debug.LogError("[FunDream_Utils] Unrecognized texture channel from shader: " + str + "\nDefaulting to Alpha"); return TextureChannel.Alpha;
		}
	}
	...
	static public int ShaderKeywordRadioGeneric( string header, int index, GUIContent[] labels )
	{
		...
		{
			r.width = FunDream_Utils.ScreenWidthRetina - 20f;
		}
...
//			EditorUtility.CopySerialized(originalMesh, newMesh);
	...
	static public void CreateArchive(PackedFile[] packedFiles, string outputFile)
	{
		...
		{
			EditorApplication.Beep();
			Debug.LogError("[FunDream_ PackFile] No file to pack!");
			return;
		}
		...
		sbIndex.AppendLine("# FunDream_ PACKED SHADERS");
		int cursor = 0;
		...
		Debug.Log("[FunDream_ CreateArchive] Created archive:\n" + fullPath);
	}
