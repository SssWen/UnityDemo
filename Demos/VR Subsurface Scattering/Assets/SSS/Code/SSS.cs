...
namespace SSS
{
    ...
    public class SSS : MonoBehaviour
    {
        ...
        protected void GetShadowsRT(ref RenderTexture rt, int x, int y, string name)
        {
            ReleaseRT(rt);
            rt = RenderTexture.GetTemporary(x, y, 16, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
            rt.filterMode = FilterMode.Point;
        ...
        #region Shadow Copy things
        CommandBuffer[] LightCMD;
        LightEvent Event = LightEvent.AfterScreenspaceMask;
        #endregion

        public RenderTexture ShadowCopy/*, ShadowCopy2*/;
        ...
        void CleanupLightCMD()
        {
            if (LightCMD != null && DirectionalLights.Count > 0)
            {
                for (int i = 0; i < DirectionalLights.Count; i++)
                {
                    DirectionalLights[i].RemoveCommandBuffer(Event, LightCMD[i]);
                    LightCMD[i].Clear();
